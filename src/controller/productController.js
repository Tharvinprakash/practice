...
exports.addProduct = async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      const prod = await knex("products").insert({
        name: name,
...
exports.getProduct = async (req, res) => {  
  let products = await knex("products as p").leftJoin("stocks as s","p.id","s.product_id").select("p.*","s.quantity");

  return res
    .status(200)
    .json({ data: products, message: "products fetch successfully" });
};
  ...
  try {
    ...
    if (product) {
      const prod = await knex("products")
        ...
        await knex("stocks").where({product_id: prod.id}).update({quantity: quantity})
    }
...
exports.pagination = async (req, res) => {
  ...
  try {
    let products = await knex("products").paginate({
      perPage: limit,
      currentPage: page,
    });
    if (!products) {
    ...
    return res
      .status(400)
      .json({ message: "Error while pagination on products" });
  }
};
...
exports.sortByProductName = async (req, res) => {
  ...
  try {
    let products = await knex("products").select("*").orderBy("name", `${op}`);
...
exports.bulkUpload = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!row.purchased_price)
        errors.purchased_price = "purchased_price is required";
      ...
      if (row.is_delete === undefined)
        errors.is_delete = "is_delete is required";
      if (!row.ratings && row.ratings !== 0)
...
exports.exportProducts = async (req, res) => {
  ...
  try {
    const products = await knex("products as p").select(
      "p.name",
      "p.name_slug",
      "p.description",
      "p.marked_price",
    ...
    const beforeBuffer = XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      "Products",
    );
